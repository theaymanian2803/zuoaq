      ...
      <div className="fixed top-0 right-0 h-full w-full max-w-md bg-background z-50 shadow-2xl animate-slide-in-right">
        <div className="flex flex-col h-full">
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {items.length === 0 ? (
              <p className="text-sm font-sans text-muted-foreground text-center mt-12">Your cart is empty</p>
            ) : (
              <div className="space-y-6">
                {items.map((item) => (
                  <div key={item.product.id} className="flex gap-4">
                    <img src={item.product.image_url} alt={item.product.name} className="w-20 h-20 object-cover" loading="lazy" width={80} height={80} />
                    <div className="flex-1">
                      <h3 className="font-serif text-sm">{item.product.name}</h3>
                      <p className="text-xs font-sans text-muted-foreground mt-1">${item.product.price}</p>
